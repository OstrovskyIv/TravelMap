  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- ПРАВЫЙ БЛОК: ИНФОРМАЦИЯ (БРИФИНГ) -->
        <div class="w-[550px] flex flex-col gap-12 p-16 overflow-y-auto custom-scrollbar">
          ...
          <div class="flex flex-col gap-4">
            ...
            <h2 class="text-6xl font-black text-white uppercase tracking-tighter italic leading-none">
              {{ countryData?.names[langStore.currentLang] }}
            </h2>
          </div>
          ...
          <div class="grid grid-cols-3 gap-4">
            <div
                v-for="(val, key) in countryData?.stats" :key="key"
                class="flex flex-col gap-1 p-5 bg-white/5 rounded-[24px] border border-white/5"
            >
          ...
          <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-2">
              <div
                  v-for="city in countryData?.cities" :key="city.id"
                  class="group flex items-center justify-between p-6 rounded-[28px] bg-white/[0.02] border border-white/5 hover:border-[#fbbf24]/40 transition-all cursor-pointer"
                  @mouseenter="highlightCity(city.id)"
                  @mouseleave="highlightCity(null)"
              >
                <div class="flex flex-col gap-1">
                  <span class="text-sm font-black text-white opacity-40 group-hover:opacity-100 group-hover:text-[#fbbf24] transition-all uppercase tracking-widest">
                    {{ city.names[langStore.currentLang] }}
                  </span>
...
const countryData = computed(() => {
  return ALL_COUNTRIES.find(c => c.id === mapStore.pendingCountryId)
})
...
const drawMiniMap = async () => {
  ...
  const projection = d3.geoMercator().fitSize([width - 150, height - 150], feature)
  ...
  if (countryData.value?.cities) {
    ...
    countryData.value.cities.forEach(city => {
      ...
      node.append('circle').attr('r', city.isCapital ? 8 : 5).attr('fill', props.theme.colors.ui.accent).attr('opacity', 0.2)
          .append('animate').attr('attributeName', 'r').attr('values', city.isCapital ? '8;14;8' : '5;10;5').attr('dur', '2s').attr('repeatCount', 'indefinite')
