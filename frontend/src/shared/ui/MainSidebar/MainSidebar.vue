  ...
  <aside class="h-full border-r flex flex-col gap-10 p-8 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] z-40 relative shadow-2xl overflow-hidden" :class="ui.isSidebarOpen ? 'w-80 opacity-100' : 'w-0 p-0 opacity-0 border-none pointer-events-none'" :style="{ backgroundColor: theme?.sidebar.bg, borderColor: theme?.sidebar.border }">
    <div class="flex flex-col gap-10 min-w-[260px] h-full">
      ...
      <nav class="flex flex-col gap-2">
        <div v-for="item in menuItems" :key="item.id" class="group flex items-center gap-5 p-4 rounded-2xl cursor-pointer transition-all duration-300 hover:translate-x-1 active:scale-95" :style="activeItem === item.id ? { backgroundColor: theme?.sidebar.activeBg } : {}">
          <span class="text-xl filter drop-shadow-md group-hover:scale-110 transition-transform">{{ item.icon }}</span>
          <span class="text-[13px] font-bold tracking-tight uppercase transition-colors" :style="activeItem === item.id ? { color: theme?.sidebar.accent } : { color: theme?.sidebar.text }">
      ...
      <section class="mt-auto flex flex-col gap-4">
        <div class="flex flex-col gap-3 p-6 rounded-[32px] border transition-all duration-500 shadow-inner" :style="{ backgroundColor: theme?.sidebar.activeBg, borderColor: theme?.sidebar.border }">
          <header class="flex justify-between items-center">
            ...
            <span class="text-[10px] font-mono font-bold" :style="{ color: theme?.sidebar.accent }">
              {{ Math.round((store.visited.length / ALL_COUNTRIES.length) * 100) }}%
            </span>
          </header>

          <div class="flex items-baseline gap-2">
            <span class="text-4xl font-black tracking-tighter" :style="{ color: theme?.sidebar.accent }">{{ store.visited.length }}</span>
        ...
        <div class="flex items-center gap-4 p-2 group cursor-pointer">
          ...
          <div class="flex flex-col">
            <span class="text-xs font-bold tracking-tight" :style="{ color: theme?.sidebar.accent }">Ivan User</span>
...
const menuItems = [
  { id: 'map', icon: '🌍', nameRu: 'Атлас', nameEn: 'World Atlas' },
  { id: 'stats', icon: '💎', nameRu: 'Достижения', nameEn: 'Achievements' },
  { id: 'settings', icon: '🔮', nameRu: 'Стилистика', nameEn: 'Environment' }
]
</script>