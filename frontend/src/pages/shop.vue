<template>
  <div class="w-full h-full p-12 flex flex-col gap-16 overflow-y-auto custom-scrollbar font-sans transition-colors duration-1000 bg-[var(--bg-main)]">
    <header class="flex justify-between items-center">
      <div class="flex flex-col gap-2 text-white">
        <h1 class="text-7xl font-black uppercase tracking-tighter italic leading-none">
          {{ langStore.t.shop.title }}
        </h1>
        <span class="text-xs font-mono uppercase tracking-[0.5em] opacity-40 text-[var(--ui-text)]">
           {{ langStore.t.shop.subtitle }}
        </span>
      </div>
      <BalanceTicket />
    </header>
    ...
    <section class="flex flex-col gap-8">
      <h2 class="text-xs font-black uppercase tracking-[0.4em] text-white opacity-30">{{ langStore.t.shop.collection }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <button v-for="theme in ownedThemes" :key="theme.id" @click="mapStore.setTheme(theme.id)"
                class="p-10 rounded-[48px] border transition-all hover:scale-[1.02] text-left outline-none cursor-pointer bg-white/5"
                :style="{ borderColor: mapStore.currentThemeId === theme.id ? 'var(--ui-accent)' : 'rgba(255,255,255,0.1)' }">
    ...
    <section v-if="availableThemes.length > 0" class="flex flex-col gap-8">
      <h2 class="text-xs font-black uppercase tracking-[0.4em] text-white opacity-30">{{ langStore.t.shop.available }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div v-for="theme in availableThemes" :key="theme.id" class="p-10 rounded-[48px] border border-white/5 bg-black/20 flex flex-col gap-6">
          ...
          <button @click="userStore.buyTheme(theme.id, theme.price || 0)"
                  ...
                  class="w-full py-4 rounded-xl font-black uppercase transition-all border-none cursor-pointer"
