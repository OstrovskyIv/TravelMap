    ...
    <header class="flex justify-between items-start">
      <div class="flex flex-col gap-6">
        ...
        <nav class="flex gap-3">
          <button v-for="tab in shopTabs" :key="tab.id" @click="activeTab = tab.id" class="px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border-none cursor-pointer transition-all italic" :class="activeTab === tab.id ? 'bg-white text-black shadow-lg shadow-white/10' : 'bg-white/5 text-white/40 hover:bg-white/10'">{{ tab.label }}</button>
        </nav>
      </div>
    ...
    <div class="flex flex-col gap-20 pb-20">
      <section v-if="purchasedItems.length > 0" class="flex flex-col gap-10">
        <h2 class="text-xs font-black uppercase tracking-[0.5em] text-white/20 italic flex items-center gap-4">
          ...
          {{ langStore.currentLang === 'ru' ? 'Приобретено' : 'Purchased' }}
        </h2>
        <div :class="activeTab === 'themes' ? 'grid grid-cols-3 gap-8' : 'grid grid-cols-5 gap-6'">
          <div v-for="item in purchasedItems" :key="item.id" class="p-8 rounded-[40px] border border-white/5 bg-white/[0.02] flex flex-col items-center gap-6 group hover:bg-white/[0.04] transition-all">
            ...
            <button @click="handleEquip(item)" class="w-full py-4 rounded-2xl text-[9px] font-black uppercase italic border-none cursor-pointer transition-all" :class="isEquipped(item) ? 'bg-[var(--ui-accent)] text-black shadow-lg shadow-[var(--ui-accent)]/20' : 'bg-white/5 text-white/60 hover:bg-white/10'">
      ...
      <section v-if="availableItems.length > 0" class="flex flex-col gap-10">
        <h2 class="text-xs font-black uppercase tracking-[0.5em] text-white/20 italic flex items-center gap-4">
          <span class="w-12 h-[1px] bg-white/10"></span>
          {{ langStore.currentLang === 'ru' ? 'Доступно в магазине' : 'Available' }}
        </h2>
        <div :class="activeTab === 'themes' ? 'grid grid-cols-3 gap-8' : 'grid grid-cols-5 gap-6'">
          <div v-for="item in availableItems" :key="item.id" class="p-8 rounded-[40px] border border-white/5 bg-black/20 flex flex-col items-center gap-6">
            ...
            <button @click="handleBuy(item)" :disabled="userStore.balance < item.price" class="w-full py-4 rounded-2xl text-[9px] font-black uppercase italic border-none cursor-pointer transition-all" :class="userStore.balance >= item.price ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20 hover:scale-105' : 'bg-white/5 text-white/10 cursor-not-allowed'">
              {{ item.price }} Credits
            </button>
          </div>
        </div>
...
const mapStore = useMapStore(); const langStore = useLangStore(); const userStore = useUserStore()
...
const emojiItems = [{ id: 'e1', val: '🚀', price: 100 }, { id: 'e2', val: '🏔️', price: 150 }, { id: 'e3', val: '🏝️', price: 200 }, { id: 'e4', val: '⛩️', price: 250 }, { id: 'e5', val: '🗼', price: 300 }]
const hatItems = [{ id: 'h1', val: '🎩', price: 500 }, { id: 'h2', val: '👑', price: 1000 }, { id: 'h3', val: '🎓', price: 400 }, { id: 'h4', val: '🤠', price: 350 }, { id: 'h5', val: '⛑️', price: 300 }]

const allItems = computed<ShopItem[]>(() => {
  if (activeTab.value === 'themes') {
    return Object.values(mapStore.themes).map(t => ({ id: t.id, title: t.name, display: '', price: t.price || 0, type: 'themes' as TabType }))
  }
  if (activeTab.value === 'profile') {
    return emojiItems.map(e => ({ id: e.id, title: '', display: e.val, price: e.price, type: 'profile' as TabType }))
  }
  if (activeTab.value === 'hats') {
    return hatItems.map(h => ({ id: h.id, title: '', display: h.val, price: h.price, type: 'hats' as TabType }))
  }
  return []
})
...
const handleEquip = (item: ShopItem) => {
  if (item.type === 'themes') mapStore.setTheme(item.id)
  if (item.type === 'profile') userStore.activeEmoji = item.display
  if (item.type === 'hats') userStore.activeHat = item.display
}

const handleBuy = (item: ShopItem) => {
  if (item.type === 'themes') userStore.buyTheme(item.id, item.price)
  if (item.type === 'profile') userStore.buyEmoji(item.display, item.price)
  if (item.type === 'hats') userStore.buyHat(item.display, item.price)
}
</script>