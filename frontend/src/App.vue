  ...
  <div class="h-screen w-screen bg-black overflow-hidden font-sans relative">
    ...
    >
      <div v-if="!uiStore.isSidebarOpen" @click="uiStore.toggleSidebar" class="absolute left-0 top-0 h-full w-14 sm:w-16 md:w-20 lg:w-24 z-40 flex items-center justify-center pointer-events-none group">
        <button class="w-10 sm:w-11 md:w-12 xl:w-14 h-auto py-6 sm:py-8 lg:py-10 flex flex-col items-center justify-center gap-4 sm:gap-6 bg-white/5 backdrop-blur-2xl border-y border-r border-white/10 rounded-r-xl sm:rounded-r-[32px] pointer-events-auto hover:w-11 sm:hover:w-13 md:hover:w-14 lg:hover:w-16 transition-all duration-500 shadow-[10px_0_30px_rgba(0,0,0,0.2)] sm:shadow-[20px_0_40px_rgba(0,0,0,0.3)] group">
          ...
          <div class="flex flex-col items-center gap-1 lg:gap-2 select-none">
          ...
          <div class="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-blue-500/40 group-hover:bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)] transition-all"></div>
        </button>
      </div>
    </Transition>
