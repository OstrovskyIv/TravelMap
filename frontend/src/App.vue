<template>
  <div class="h-screen w-screen bg-black overflow-hidden font-sans relative">
    ...
    >
      ...
      <div v-if="!uiStore.isSidebarOpen" @click="uiStore.toggleSidebar" class="absolute left-0 top-0 h-full w-20 z-40 flex flex-col items-center justify-center pointer-events-none group">
        <button class="w-12 h-auto py-10 flex flex-col items-center justify-center gap-8 bg-[#18181b]/80 backdrop-blur-3xl border-y border-r border-white/10 rounded-r-[40px] pointer-events-auto hover:w-14 transition-all duration-500 shadow-2xl group border-none outline-none">
          ...
          <span class="w-1.5 h-1.5 rounded-full shadow-[0_0_15px_rgba(251,191,36,0.4)] transition-all" :style="{ backgroundColor: mapStore.currentTheme?.colors.ui.accent }"></span>
        </button>
      </div>
    </Transition>
