<template>
  <div class="w-full h-full p-6 lg:p-10 flex flex-col gap-8 overflow-y-auto custom-scrollbar text-white">
    <div class="w-full flex flex-col shadow-2xl rounded-[32px] overflow-hidden shrink-0 transition-all">
      ...
      <div class="bg-white flex flex-col md:flex-row relative text-slate-900">
        ...
        <div class="flex-1 p-6 flex flex-wrap justify-between items-center gap-6">
          <div class="flex flex-col gap-4 min-w-[200px]">
            <div class="flex flex-col">
              <span class="text-[8px] font-black uppercase text-slate-400 tracking-[0.3em]">{{ langStore.t.admin.totalMiles }}</span>
              <div class="flex items-baseline gap-2">
                <span class="text-5xl lg:text-7xl font-black tracking-tighter">{{ userStore.balance }}</span>
                <span class="text-lg font-black text-blue-600 uppercase italic">{{ langStore.t.shop.miles }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-6 md:border-l md:border-slate-100 md:pl-8">
            <div class="flex flex-col items-center gap-1">
              <span class="text-[8px] font-black text-slate-300 uppercase leading-none">{{ langStore.t.admin.status }}</span>
      ...
      <button @click="userStore.balance = 0" class="bg-slate-50 py-3 text-[8px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-red-600 hover:bg-red-50 transition-all border-t border-slate-100 outline-none cursor-pointer">
        {{ langStore.t.admin.terminate }}
      </button>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 w-full pb-10">
      <div class="p-6 bg-white/5 border border-white/10 rounded-[32px] flex flex-col gap-4">
        <span class="text-[9px] font-black uppercase tracking-[0.2em] opacity-30 leading-none">{{ langStore.t.admin.quickInjection }}</span>
      ...
      <div class="p-6 bg-white/5 border border-white/10 rounded-[32px] flex flex-col gap-4">
        <span class="text-[9px] font-black uppercase tracking-[0.2em] opacity-30 leading-none">{{ langStore.t.admin.manualOverwrite }}</span>
        <div class="flex gap-2">
          ...
          <button @click="setExactBalance" class="px-4 py-3 rounded-xl bg-blue-600 text-white font-black text-[10px] uppercase hover:bg-blue-500 active:scale-95 transition-all shadow-lg border-none shrink-0 cursor-pointer">
            {{ langStore.t.admin.sync }}
          </button>
        </div>
      </div>
    </div>
