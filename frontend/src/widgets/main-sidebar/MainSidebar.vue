    ...
    <aside v-if="ui.isSidebarOpen" :class="['fixed z-50 flex flex-col shadow-2xl backdrop-blur-3xl border-[var(--sidebar-border)] bg-[var(--sidebar-bg)]/95', isMobile ? 'inset-x-0 bottom-0 h-[85vh] rounded-t-[40px] border-t' : 'left-0 top-0 bottom-0 w-80 rounded-r-[48px] border-r']">
      <div class="flex flex-col h-full p-8 md:p-10 gap-8 overflow-y-auto custom-scrollbar font-sans text-white">
        ...
        <header class="flex items-center justify-between">
          ...
          <button @click="ui.toggleSidebar" class="w-10 h-10 flex items-center justify-center rounded-xl bg-white/5 border-none cursor-pointer text-white/40 hover:text-white transition-all">
            {{ isMobile ? '✕' : '◀' }}
          </button>
        </header>
        ...
        <nav class="flex flex-col gap-2">
          <RouterLink v-for="item in menuItems" :key="item.id" :to="item.path"
                      class="group flex items-center gap-5 p-5 rounded-[24px] transition-all border border-transparent hover:bg-white/5"
                      :class="route.path === item.path ? 'bg-white/5 border-white/10' : ''">
            <span class="text-2xl group-hover:scale-110 transition-transform">{{ item.icon }}</span>
            <span class="text-[13px] font-black uppercase tracking-widest" :style="{ color: route.path === item.path ? 'var(--ui-accent)' : 'var(--ui-text-muted)' }">
          ...
          <button @click="ui.setGamesModal(true)" class="flex items-center gap-5 p-5 rounded-[24px] bg-transparent border-none cursor-pointer transition-all hover:bg-white/5 group">
            ...
            <span class="text-[13px] font-black uppercase tracking-widest text-[var(--ui-text-muted)] group-hover:text-white">
              {{ langStore.t.navigation.games }}
            </span>
          </button>

          <!-- КОНСОЛЬ АДМИНА -->
          <button v-if="userStore.isAdmin" @click="ui.toggleAdminConsole"
                  class="flex items-center gap-5 p-5 rounded-[24px] border border-dashed border-white/10 bg-transparent cursor-pointer transition-all hover:border-[var(--ui-accent)]/50 group"
            ...
            <span class="text-[13px] font-black uppercase tracking-widest" :style="{ color: ui.isAdminConsoleOpen ? 'var(--ui-accent)' : 'var(--ui-text-muted)' }">
              {{ langStore.t.navigation.console }}
            </span>
          </button>
        </nav>

        <!-- КНОПКА КУПИТЬ VIP (Скрывается если уже VIP) -->
        <div v-if="!userStore.isVip" class="relative p-[1px] rounded-2xl overflow-hidden cursor-pointer active:scale-95 transition-all group" @click="ui.setVipModal(true)">
        ...
        <section class="flex flex-col gap-3">
          <RouterLink to="/profile" class="flex items-center gap-5 p-4 rounded-[32px] bg-white/5 border border-white/5 shadow-inner hover:bg-white/[0.08] transition-all no-underline" :class="userStore.isVip ? 'border-yellow-500/30' : ''">
            <div class="relative shrink-0">
              <!-- Шляпа -->
              <div class="absolute -top-6 left-1/2 -translate-x-1/2 text-2xl drop-shadow-2xl z-10" v-if="userStore.activeHat">
                {{ userStore.activeHat }}
              </div>
              <!-- Аватар -->
              <div class="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg bg-[var(--ui-accent)] text-black relative z-0 uppercase"
                   :class="userStore.isVip ? 'bg-gradient-to-br from-yellow-400 to-yellow-600' : ''">
                {{ userStore.userName.charAt(0) }}
              </div>
              <!-- Эмодзи -->
              <div v-if="userStore.activeEmoji" class="absolute -bottom-1 -right-1 text-xs bg-black/80 rounded-full w-5 h-5 flex items-center justify-center border border-white/10">
                {{ userStore.activeEmoji }}
              </div>
            </div>

            <div class="flex flex-col gap-1 leading-none overflow-hidden">
              <div class="flex items-center gap-2">
                <span class="text-sm font-black italic text-white truncate">{{ userStore.userName }}</span>
                <!-- VIP Badge -->
                <span v-if="userStore.isVip" class="text-[7px] bg-yellow-500 text-black px-1 rounded-[4px] font-black tracking-tighter">VIP</span>
              </div>
              <span class="text-[8px] uppercase font-mono opacity-30 tracking-tighter text-white">View Data Node</span>
            </div>
          </RouterLink>

          <!-- КНОПКА ВЫХОДА -->
          <button @click="handleLogout" class="flex items-center gap-4 px-6 py-4 rounded-2xl bg-red-500/5 hover:bg-red-500/20 border border-red-500/10 hover:border-red-500/40 transition-all cursor-pointer group">
            <span class="text-lg opacity-40 group-hover:opacity-100 group-hover:scale-110 transition-all">⏻</span>
            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-red-500/60 group-hover:text-red-500 italic">Terminate</span>
          </button>
        </section>
...
// Список меню, ключи id должны совпадать с ключами в langStore.t.navigation
const menuItems = [
  { id: 'atlas', path: '/', icon: '🌍' },
...
<style scoped>
.vip-shimmer {
  background: linear-gradient(115deg, transparent 0%, #facc15 50%, transparent 100%);
